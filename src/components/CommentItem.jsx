...
import { createReply, getCommentReplies, toggleCommentLike } from '../api/postsApi';
import { useToast } from '../context/ToastContext';
import { getErrorMessage, getUserId } from '../utils/errorMessage';

function CommentItem({ comment, postId, currentUserId, onDelete, onUpdate, onLike, onPostRefresh }) {
  ...
  const createdLabel = comment?.createdAt ? new Date(comment.createdAt).toLocaleString() : '';
    ...
    <div className="rounded-xl border border-slate-700 bg-slate-800 p-3">
      ...
      {isEditing ? (
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <button className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-medium text-white transition hover:bg-blue-700" onClick={saveEdit}>
              Save
            </button>
            <button
        ...
        <p className="mb-2 whitespace-pre-wrap text-sm text-slate-100">{comment?.content || comment?.body || 'No text'}</p>
      )}

      {canManage && !isEditing ? (
        <div className="mt-2 flex flex-wrap gap-1.5">
          <button className="rounded-lg border border-slate-600 bg-slate-700 px-3 py-1.5 text-xs font-medium text-slate-200 transition hover:bg-slate-600" onClick={() => setIsEditing(true)}>
            Edit
          </button>
          <button className="rounded-lg bg-red-600 px-3 py-1.5 text-xs font-medium text-white transition hover:bg-red-700" onClick={() => onDelete(commentId)}>
            Delete
          </button>
          <button className="rounded-lg border border-slate-600 bg-slate-700 px-3 py-1.5 text-xs font-medium text-slate-200 transition hover:bg-slate-600" onClick={() => onLike(commentId)}>
            Like ({comment?.likesCount || comment?.likes?.length || 0})
          </button>
          <button className="rounded-lg border border-slate-600 bg-slate-700 px-3 py-1.5 text-xs font-medium text-slate-200 transition hover:bg-slate-600" onClick={toggleRepliesPanel}>
      ...
      {repliesOpen ? (
        <div className="mt-3 border-l border-slate-600 pl-3 sm:pl-4">
          <h5 className="text-xs font-semibold uppercase tracking-wide text-slate-300">Replies</h5>
          <form className="mt-2 space-y-2" onSubmit={addReply}>
            ...
            <div className="flex justify-end">
              <button className="rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-medium text-white transition hover:bg-blue-700" disabled={addingReply}>
          ...
          <div className="mt-3 space-y-2">
            {repliesLoading ? <p className="text-xs text-slate-400">Loading replies...</p> : null}
            {!repliesLoading && replies.length === 0 ? (
              <p className="text-xs text-slate-400">No replies yet.</p>
            ) : null}
            {replies.map((reply) => {
              const replyCreatedLabel = reply?.createdAt ? new Date(reply.createdAt).toLocaleString() : '';
              return (
                <article key={reply?._id || reply?.id} className="rounded-lg border border-slate-700 bg-slate-700 p-2.5">
                  <p className="text-xs font-medium text-slate-200">
                    {reply?.commentCreator?.name || reply?.replyCreator?.name || reply?.user?.name || 'User'}
                  </p>
                  <p className="mt-1 whitespace-pre-wrap text-sm text-slate-100">
                    {reply?.content || reply?.body || 'No text'}
                  </p>
                  <p className="mt-1 text-[11px] text-slate-400">{replyCreatedLabel}</p>
