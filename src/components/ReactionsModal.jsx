...
function ReactionsModal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-xl rounded-2xl border border-slate-700 bg-slate-800 p-4 shadow-2xl"
      ...
      >
        ...
        <div className="max-h-[50vh] space-y-2 overflow-y-auto pr-1">
          {loading ? (
            <p className="text-sm text-slate-400">Loading reactions...</p>
          ) : null}
          ...
          {!loading
            ? users.map((user) => (
                <article
                  ...
                  className="flex items-center gap-3 rounded-lg border border-slate-700 bg-slate-700 px-3 py-2"
                >
                  <img
                    src={
                      user?.photo ||
